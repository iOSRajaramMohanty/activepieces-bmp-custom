...
exports.debugLog = exports.CONFIG = exports.getConfig = exports.API_ENDPOINTS = exports.getBaseUrl = void 0;
exports.extractApiToken = extractApiToken;
 ...
 */
const getBaseUrl = (metadata, auth) => {
    ...
    throw new Error(`No API URL configured for ${authEnvironment || 'selected'} environment. ` +
...
exports.API_ENDPOINTS = {
     ...
     */
    validateToken: (metadata, auth) => `${(0, exports.getBaseUrl)(metadata, auth)}/user/checkToken`,
    /**
     * Get available channels endpoint
     * GET /account
     */
    getChannels: (metadata, auth) => `${(0, exports.getBaseUrl)(metadata, auth)}/account`,
    /**
     * Get accounts for a specific platform
     * GET /account?platform={platform}
     */
    getAccounts: (platform, metadata, auth) => `${(0, exports.getBaseUrl)(metadata, auth)}/account?platform=${platform}`,
    /**
     * Send message endpoint
     * POST /message
     */
    sendMessage: (metadata, auth) => `${(0, exports.getBaseUrl)(metadata, auth)}/message`,
    /**
     * Get recipients/conversations for an account
     * GET /msglog/live?accountNo={accountNo}&platform={platform}
     */
    getRecipients: (accountNo, platform, metadata, auth) => `${(0, exports.getBaseUrl)(metadata, auth)}/msglog/live?accountNo=${accountNo}&platform=${platform}`,
    /**
     * Get contact categories for a platform
     * GET /contact-category?platform={platform}
     */
    getContactCategories: (platform, metadata, auth) => `${(0, exports.getBaseUrl)(metadata, auth)}/contact-category?platform=${platform}`,
    /**
     * Send bulk message endpoint
     * POST /contact-category/bulkmessage
     */
    sendBulkMessage: (metadata, auth) => `${(0, exports.getBaseUrl)(metadata, auth)}/contact-category/bulkmessage`,
    /**
     * Get templates for a specific account
     * GET /account/{accountId}/template
     */
    getTemplates: (accountId, metadata, auth) => `${(0, exports.getBaseUrl)(metadata, auth)}/account/${accountId}/template`,
};
/**
 ...
 */
const debugLog = (message, data, metadata) => {
    const config = (0, exports.getConfig)(metadata);
    if (config.debug) {
 ...
 */
function getAuthToken(server, auth) {
 ...
 */
async function fetchMetadata(projectId, server, httpClient, auth) {
    try {
        ...
        console.log('[ADA-BMP Config] Looking for environment:', {
            ...
            availableEnvironments: environmentsResponse.body.map((e) => ({
                name: e.environment,
                hasMetadata: !!e.metadata,
                apiUrl: e.metadata?.ADA_BMP_API_URL
            }))
        });
        const matchingEnv = environmentsResponse.body.find((env) => env.environment === environment);
        if (matchingEnv?.metadata) {
            console.log('[ADA-BMP Config] ✅ Found environment-specific metadata:', {
                environment,
                hasMetadata: !!matchingEnv.metadata,
                apiUrl: matchingEnv.metadata?.ADA_BMP_API_URL,
            });
            return matchingEnv.metadata;
        }
        ...
        if (orgResponse.body?.metadata) {
            ...
            return orgResponse.body.metadata;
        }
