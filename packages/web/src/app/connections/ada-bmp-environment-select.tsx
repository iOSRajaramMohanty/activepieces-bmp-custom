  ...
  ({
    ...
    useEffect(() => {
      const fetchAllowedEnvironments = async () => {
        try {
          console.log(
            '[ADA BMP Environment Select] Fetching allowed environments...',
          );

          // Call the new backend endpoint to get allowed environments
          const response =
            await organizationApi.getAllowedEnvironmentsForCurrentUser();

          console.log(
            '[ADA BMP Environment Select] Allowed environments:',
          ...
          console.log(
            '[ADA BMP Environment Select] Filtered options:',
          ...
          if (filtered.length > 0) {
            ...
            console.warn(
              '[ADA BMP Environment Select] No matching environments, showing all',
            );
            setFilteredOptions(allOptions);
          }
        } catch (error) {
          console.error(
            '[ADA BMP Environment Select] Error fetching allowed environments:',
            error,
          );
