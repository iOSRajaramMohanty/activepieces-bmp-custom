...
export default function OrganizationsPage() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = organizationHooks.useOrganizations(platform.id);
  ...
  const columns: ColumnDef<RowDataWithActions<Organization>, unknown>[] =
    useMemo(
      () => [
        {
          accessorKey: 'name',
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      <DashboardPageHeader
        ...
        description={t(
          'Manage organizations and their environment-specific metadata',
        )}
      />
        ...
        <>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {t(
                  'View organization-level metadata and configure environment-specific metadata. Organization-level metadata is read-only (from system configuration), while environment-specific metadata can be configured to override defaults.',
                )}
                <br />
                <span className="text-xs text-muted-foreground mt-1 block">
                  {t(
                    'Note: You can only view and configure metadata for your own organization. Organization owners and super admins can access all organizations.',
                  )}
                </span>
              </CardDescription>
            </CardHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm text-muted-foreground">
              <p>
                {t(
                  'Organization metadata allows you to configure environment-specific settings that apply to all projects within an organization.',
                )}
              </p>
              ...
              <ul className="list-disc list-inside space-y-1 ml-2">
                <li>
                  ...
                  {t(
                    'Read-only default metadata from system configuration. This is the fallback API URL when no environment-specific metadata is configured.',
                  )}
                </li>
                <li>
                  <strong>{t('Environment-Level:')}</strong>{' '}
                  {t(
                    'Environment-specific metadata that overrides organization defaults. Configure via the expandable sections below each organization.',
                  )}
                </li>
              </ul>
              <p className="pt-2">
                {t(
                  'Environment-specific metadata takes precedence over organization-level metadata. Use different API URLs and settings per environment (Dev, Staging, Production) for complete isolation.',
                )}
              </p>
            </CardContent>
          </Card>
