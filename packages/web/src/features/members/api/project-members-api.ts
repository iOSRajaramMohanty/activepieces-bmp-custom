...
export const projectMembersApi = {
  async list(request: ListProjectMembersRequestQuery) {
    try {
      const response = await api.get<SeekPage<ProjectMemberWithUser> | { statusCode: number; error: string; message: string }>(
        '/v1/project-members',
        request,
        {
      ...
      if (response && typeof response === 'object' && 'statusCode' in response && response.statusCode === 404) {
        return { data: [], next: null, previous: null } as SeekPage<ProjectMemberWithUser>;
      }
