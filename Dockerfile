...
# Remove bun.lock first — COPY . . overwrites the fresh one generated by bun install with the
# original repo copy which may contain duplicate-key errors that cause bun install to fail.
RUN --mount=type=cache,target=/root/.bun/install/cache \
    rm -f bun.lock && npx nx build pieces-ada-bmp --skip-nx-cache
